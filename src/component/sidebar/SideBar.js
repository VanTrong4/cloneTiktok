...
import { Row, Space, Button, Avatar, Tag } from "antd";
import { CheckCircleOutlined, NumberOutlined } from "@ant-design/icons";
import { useEffect, useState } from "react";
import suggestUser from "../../api/suggestUser";
function SideBar(params) {
  const Identify = localStorage.getItem('log')
  ...
  useEffect(() => {
    const getSuggest = async () => {
      try {
        ...
        const respones = await suggestUser.getAll(params);
        setSuggest(respones.data);
      } catch (e) {
        console.log(e);
      }
    };
    ...
    >
      ...
      >
        ...
        <Button
          type="primary"
          ghost
          style={{ marginTop: 20, minHeight: 48, color: "primary" }}
          block
        >
          Đăng nhập
        </Button>
      ...
      >
        <Row style={{ marginBottom: 15 }}>Tài khoản được đề xuất</Row>
        {console.log(suggest)}
        {suggest.map((val) => {
          return (
            <Button
            ...
            >
              <Avatar
                ...
                src={val.avatar}
              />
              <Space style={{ display: "block", marginLeft: "15px" }}>
                <Row
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  {val.nickname}
                  {val.tick && <CheckCircleOutlined style={{ color: "rgb(32 213 236)" }} />}
                </Row>
                <Row style={{ fontSize: "13px" }}>{val.first_name +" "+ val.last_name }</Row>
              </Space>
            </Button>
          );
        })}
      ...
      {Identify=='true' ? <Row
        ...
        style={{ padding: "16px 0", position: "relative" }}
      >
        <Row style={{ marginBottom: 15 }}>Các tài khoản đang following</Row>
        {console.log(suggest)}
        {suggest.map((val) => {
          return (
            <Button
      ...
      >
        <Row style={{ marginBottom: "15px" }}>Khám phá</Row>
        <Row>
          <Tag>
            <NumberOutlined />
            <a href="#">Suthatla</a>
      ...
      >
        <Row style={{ marginBottom: "15px" }}>
          Giới thiệuTikTok BrowseBảng tinLiên hệSự nghiệpByteDance
        </Row>
        <Row>TikTok for GoodQuảng cáoDevelopersTransparencyTikTok Rewards</Row>
      </Row>
    </Space>
  );
}
